...
import org.lntu.online.server.capture.UnpassCourseCapture;
...
import org.lntu.online.server.model.UnpassCourse;
...
public class UnpassCourseResource {

    @GET
    @Path("{studentId}")
    @Authorization
    public List<UnpassCourse> getUnpassCourse(
        ...
        if (user.getType() == UserType.ADMINISTRATOR) { // 管理员用户
            try {
                ...
                return UnpassCourseCapture.getUnpassCourseList(targetUser.getId(), targetUser.getPassword());
            } catch (Exception e) {
            ...
            if ("~self".equals(studentId) || user.getId().equals(studentId)) {
                return UnpassCourseCapture.getUnpassCourseList(user.getId(), user.getPassword());
            } else {
