...
import org.lntu.online.server.capture.GradesCapture;
...
import org.lntu.online.server.model.Grades;
    ...
    public Grades getGrades(
        ...
        if (user.getType() == UserType.ADMINISTRATOR) { // 管理员用户
            try {
                ...
                return GradesCapture.getGrades(targetUser.getId(), targetUser.getPassword());
            } catch (Exception e) {
            ...
            if ("~self".equals(studentId) || user.getId().equals(studentId)) {
                return GradesCapture.getGrades(user.getId(), user.getPassword());
            } else {
